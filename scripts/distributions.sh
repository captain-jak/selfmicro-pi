...
install_common()
{
	...
	EOF
		...
		if [[ -n $BOOTENV_FILE ]]; then
			if [[ -f $USERPATCHES_PATH/bootenv/$BOOTENV_FILE ]]; then
				cp "$USERPATCHES_PATH/bootenv/${BOOTENV_FILE}" "${SDCARD}"/boot/selfmicroEnv.txt
			elif [[ -f $EXTER/config/bootenv/$BOOTENV_FILE ]]; then
				cp "${EXTER}/config/bootenv/${BOOTENV_FILE}" "${SDCARD}"/boot/selfmicroEnv.txt
			fi
		fi

		# TODO: modify $bootscript_dst or selfmicroEnv.txt to make NFS boot universal
		...
		[[ -n $OVERLAY_PREFIX && -f "${SDCARD}"/boot/selfmicroEnv.txt && ($BRANCH =~ current|next || $BOARDFAMILY == "rockchip-rk3588") ]] && \
			echo "overlay_prefix=$OVERLAY_PREFIX" >> "${SDCARD}"/boot/selfmicroEnv.txt

		[[ -n $DEFAULT_OVERLAYS && -f "${SDCARD}"/boot/selfmicroEnv.txt && ($BRANCH =~ current|next || $BOARDFAMILY == "rockchip-rk3588") ]] && \
			echo "overlays=${DEFAULT_OVERLAYS//,/ }" >> "${SDCARD}"/boot/selfmicroEnv.txt

		[[ -n $BOOT_FDT_FILE && -f "${SDCARD}"/boot/selfmicroEnv.txt ]] && \
			echo "fdtfile=${BOOT_FDT_FILE}" >> "${SDCARD}/boot/selfmicroEnv.txt"
	...
	[[ "${BOOTCONFIG}" != "none" ]] && {
		if [[ "${REPOSITORY_INSTALL}" != *u-boot* ]]; then
			...
			UBOOT_VER=$(dpkg --info "${DEB_SELFMICRO}/u-boot/${CHOSEN_UBOOT}_${REVISION}_${ARCH}.deb" | grep Descr | awk '{print $(NF)}')
			install_deb_chroot "${DEB_SELFMICRO}/u-boot/${CHOSEN_UBOOT}_${REVISION}_${ARCH}.deb" "selfmicro"
		fi
	}
...
PRE_INSTALL_KERNEL_DEBS
	...
	[[ -n $KERNELSOURCE ]] && {
		if [[ "${REPOSITORY_INSTALL}" != *kernel* ]]; then
			...
			VER=$(dpkg --info "${DEB_SELFMICRO}/${CHOSEN_KERNEL}_${REVISION}_${ARCH}.deb" | grep Descr | awk '{print $(NF)}')
			VER="${VER/-$LINUXFAMILY/}"

			install_deb_chroot "${DEB_SELFMICRO}/${CHOSEN_KERNEL}_${REVISION}_${ARCH}.deb" "selfmicro"

			if [[ -f ${DEB_SELFMICRO}/${CHOSEN_KERNEL/image/dtb}_${REVISION}_${ARCH}.deb ]]; then
				install_deb_chroot "${DEB_SELFMICRO}/${CHOSEN_KERNEL/image/dtb}_${REVISION}_${ARCH}.deb" "selfmicro"
			fi

			if [[ $INSTALL_HEADERS == yes ]]; then
				install_deb_chroot "${DEB_SELFMICRO}/${CHOSEN_KERNEL/image/headers}_${REVISION}_${ARCH}.deb" "selfmicro"
			fi
		fi
	}
...
POST_INSTALL_KERNEL_DEBS
	...
	if [[ "${REPOSITORY_INSTALL}" != *bsp* ]]; then
		...
		install_deb_chroot "${DEB_SELFMICRO}/$RELEASE/${CHOSEN_ROOTFS}_${BSP_CLI_PACKAGE_FULLNAME}.deb" "selfmicro"
	fi

	# install selfmicro-desktop
	if [[ "${REPOSITORY_INSTALL}" != *selfmicro-desktop* ]]; then
		...
		if [[ $BUILD_DESKTOP == yes ]]; then
			install_deb_chroot "${CHOSEN_DESKTOP}" "selfmicro"
			# install display manager and PACKAGE_LIST_DESKTOP_FULL packages if enabled per board
			desktop_postinstall
		fi
	fi

	# install selfmicro-firmware
	if [[ "${REPOSITORY_INSTALL}" != *selfmicro-firmware* ]]; then
		if [[ -f ${DEB_STORAGE}/selfmicro-firmware_${REVISION}_all.deb ]]; then
			install_deb_chroot "${DEB_STORAGE}/selfmicro-firmware_${REVISION}_all.deb"
		fi
	else
		if [[ -f ${DEB_STORAGE}/selfmicro-firmware_${REVISION}_all.deb ]]; then
			install_deb_chroot "${DEB_SELFMICRO}/selfmicro-firmware_${REVISION}_all.deb" "selfmicro"
		fi
	fi

	# install selfmicro-config
	if [[ "${PACKAGE_LIST_RM}" != *selfmicro-config* ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *selfmicro-config* ]]; then
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_STORAGE}/selfmicro-config_${REVISION}_all.deb"
			fi
		else
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_SELFMICRO}/selfmicro-config_${REVISION}_all.deb" "selfmicro"
			fi
		fi
	fi

	# install selfmicro-zsh
	if [[ "${PACKAGE_LIST_RM}" != *selfmicro-zsh* ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *selfmicro-zsh* ]]; then
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_STORAGE}/selfmicro-zsh_${REVISION}_all.deb"
			fi
		else
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "selfmicro-zsh" "remote"
			fi
		fi
	fi

	# install plymouth-theme-selfmicro
	if [[ $PLYMOUTH == yes && $BUILD_DESKTOP == yes && $RELEASE != buster ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *plymouth-theme-selfmicro* ]]; then
			install_deb_chroot "${DEB_STORAGE}/selfmicro-plymouth-theme_${REVISION}_all.deb"
		else
			install_deb_chroot "selfmicro-plymouth-theme" "remote"
		fi
	fi
	...
	cp "${EXTER}"/packages/blobs/splash/selfmicro-u-boot.bmp "${SDCARD}"/boot/boot.bmp
...
FAMILY_TWEAKS
	...
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-firstrun.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-firstrun-config.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-zram-config.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-hardware-optimize.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-ramlog.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-resize-filesystem.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable selfmicro-hardware-monitor.service >/dev/null 2>&1"

	# copy "first run automated config, optional user configured"
 	cp ${EXTER}/packages/bsp/selfmicro_first_run.txt.template "${SDCARD}"/boot/selfmicro_first_run.txt.template

	## switch to beta repository at this stage if building nightly images
	#[[ $IMAGE_TYPE == nightly ]] \
	#&& echo "deb http://beta.selfmicro.com $RELEASE main ${RELEASE}-utils ${RELEASE}-desktop" \
	#> "${SDCARD}"/etc/apt/sources.list.d/selfmicro.list
	...
	# save initial selfmicro-release state
	cp "${SDCARD}"/etc/selfmicro-release "${SDCARD}"/etc/selfmicro-image-release
...
install_distribution_specific()
{
	...
	bionic|focal|hirsute|impish|jammy)
			...
			[[ -d "${SDCARD}"/etc/netplan ]] && cat <<-EOF > "${SDCARD}"/etc/netplan/selfmicro-default.yaml
			network:
			  version: 2
			  renderer: $RENDERER
			EOF
