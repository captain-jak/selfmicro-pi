...
# cleanup
rm -f /run/selfmicro/*.pid
mkdir -p /run/selfmicro
...
build_main ()
{
	...
	touch "/run/selfmicro/${BOARD^}_${BRANCH}_${RELEASE}_${BUILD_DESKTOP}_${BUILD_MINIMAL}.pid";
	...
	echo -e "\n"
	rm "/run/selfmicro/${BOARD^}_${BRANCH}_${RELEASE}_${BUILD_DESKTOP}_${BUILD_MINIMAL}.pid"
}
	...
	while read -r line; do
		...
		if [[ "${BUILD_STABILITY}" == "${STABILITY}" ]]; then
			...
			if [[ $1 != "dryrun" ]] && [[ $n -ge $START ]]; then
				...
				while :
				do
					[[ $(find /run/selfmicro/*.pid 2>/dev/null | wc -l) -le ${MULTITHREAD} || -z ${MULTITHREAD} ]] && break
					sleep 5
				done
