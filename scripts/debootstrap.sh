...
PRE_INSTALL_DISTRIBUTION_SPECIFIC
	...
	# install locally built packages or install pre-built packages from selfmicro
...
PREPARE_IMAGE_SIZE
	...
	if [[ -f $SDCARD/boot/selfmicroEnv.txt ]]; then
		if [[ $CRYPTROOT_ENABLE == yes ]]; then
			echo "rootdev=$rootdevice cryptdevice=UUID=$(blkid -s UUID -o value ${LOOP}p${rootpart}):$ROOT_MAPPER" >> $SDCARD/boot/selfmicroEnv.txt
		else
			echo "rootdev=$rootfs" >> $SDCARD/boot/selfmicroEnv.txt
		fi
		echo "rootfstype=$ROOTFS_TYPE" >> $SDCARD/boot/selfmicroEnv.txt
	...
	# if we have boot.ini = remove selfmicroEnv.txt and add UUID there if enabled
	if [[ -f $SDCARD/boot/boot.ini ]]; then
		...
		if [[ $LINUXFAMILY != meson64 ]]; then
			[[ -f $SDCARD/boot/selfmicroEnv.txt ]] && rm $SDCARD/boot/selfmicroEnv.txt
		fi
	fi

	# if we have a headless device, set console to DEFAULT_CONSOLE
	if [[ -n $DEFAULT_CONSOLE && -f $SDCARD/boot/selfmicroEnv.txt ]]; then
		if grep -lq "^console=" $SDCARD/boot/selfmicroEnv.txt; then
			sed -i "s/^console=.*/console=$DEFAULT_CONSOLE/" $SDCARD/boot/selfmicroEnv.txt
		else
			echo "console=$DEFAULT_CONSOLE" >> $SDCARD/boot/selfmicroEnv.txt
		fi
	fi
