...
create_board_package()
{
	...
	if [[ "${BOOTCONFIG}" != "none" ]]; then
		...
		mkdir -p "${destination}"/usr/share/selfmicro/

		# create extlinux config file
		if [[ $SRC_EXTLINUX != yes ]]; then
			if [ -f "${USERPATCHES_PATH}/bootscripts/${bootscript_src}" ]; then
			  cp "${USERPATCHES_PATH}/bootscripts/${bootscript_src}" "${destination}/usr/share/selfmicro/${bootscript_dst}"
			else
			  cp "${EXTER}/config/bootscripts/${bootscript_src}" "${destination}/usr/share/selfmicro/${bootscript_dst}"
			fi
			[[ -n $BOOTENV_FILE && -f $SRC/config/bootenv/$BOOTENV_FILE ]] && \
				cp "${EXTER}/config/bootenv/${BOOTENV_FILE}" "${destination}"/usr/share/selfmicro/selfmicroEnv.txt
		fi
	...
	# Depends: fping is needed for selfmicromonitor to upload selfmicro-hardware-monitor.log
	...
	Suggests: selfmicro-config
	Replaces: zram-config, base-files, selfmicro-tools-$RELEASE, linux-${RELEASE}-root-legacy-$BOARD (<< $REVISION~), linux-${RELEASE}-root-current-$BOARD (<< $REVISION~), linux-${RELEASE}-root-next-$BOARD (<< $REVISION~)
	...
	*)
	...
	[ -f "/usr/lib/selfmicro/apt-updates" ] && rm /usr/lib/selfmicro/apt-updates
	[ -f "/usr/lib/selfmicro/firstrun-config.sh" ] && rm /usr/lib/selfmicro/firstrun-config.sh
	...
	if [ remove = "\$1" ] || [ abort-install = "\$1" ]; then
	    ...
	    systemctl disable selfmicro-hardware-monitor.service selfmicro-hardware-optimize.service >/dev/null 2>&1
	    systemctl disable selfmicro-zram-config.service selfmicro-ramlog.service >/dev/null 2>&1
	...
	[ -f /etc/lib/systemd/system/selfmicro-ramlog.service ] && systemctl --no-reload enable selfmicro-ramlog.service
	...
	if [ -n "\$(grep -w '^ENABLED=false' /etc/default/log2ram 2> /dev/null)" ]; then
	     ...
	     sed -i "s/^ENABLED=.*/ENABLED=false/" /etc/default/selfmicro-ramlog
	...
	cat <<-EOF >> "${destination}"/DEBIAN/postinst
    # move bootscript to /usr/share/selfmicro
    # create a backup
    [ -f /etc/selfmicro-release ] &&  . /etc/selfmicro-release
    [ -z \${VERSION} ] && VERSION=$(echo \`date +%s\`)
    if [ -f /boot/$bootscript_dst ]; then
       cp /boot/$bootscript_dst /usr/share/selfmicro/${bootscript_dst}-\${VERSION} >/dev/null 2>&1
       echo "NOTE: You can find previous bootscript versions in /usr/share/selfmicro !"
    fi

    # cleanup old bootscript backup
    ls /usr/share/selfmicro/boot.cmd-* >/dev/null 2>&1 | head -n -5 | xargs rm -f --
    ls /usr/share/selfmicro/boot.ini-* >/dev/null 2>&1 | head -n -5 | xargs rm -f --

    echo "Recreating boot script"
    cp /usr/share/selfmicro/$bootscript_dst /boot  >/dev/null 2>&1
    rootdev=\$(sed -e 's/^.*root=//' -e 's/ .*\$//' < /proc/cmdline)
    rootfstype=\$(sed -e 's/^.*rootfstype=//' -e 's/ .*$//' < /proc/cmdline)

    # recreate selfmicroEnv.txt if it and extlinux does not exists
    if [ ! -f /boot/selfmicroEnv.txt ] && [ ! -f /boot/extlinux/extlinux.conf ]; then
      cp /usr/share/selfmicro/selfmicroEnv.txt /boot  >/dev/null 2>&1
      echo "rootdev="\$rootdev >> /boot/selfmicroEnv.txt
      echo "rootfstype="\$rootfstype >> /boot/selfmicroEnv.txt
    fi
	...
	if [ ! -f "/etc/default/selfmicro-motd" ]; then
		mv /etc/default/selfmicro-motd.dpkg-dist /etc/default/selfmicro-motd
	fi
	if [ ! -f "/etc/default/selfmicro-ramlog" ] && [ -f /etc/default/selfmicro-ramlog.dpkg-dist ]; then
		mv /etc/default/selfmicro-ramlog.dpkg-dist /etc/default/selfmicro-ramlog
	fi
	if [ ! -f "/etc/default/selfmicro-zram-config" ] && [ -f /etc/default/selfmicro-zram-config.dpkg-dist ]; then
		mv /etc/default/selfmicro-zram-config.dpkg-dist /etc/default/selfmicro-zram-config
	fi
	...
	# Reload services
	systemctl --no-reload enable selfmicro-hardware-monitor.service selfmicro-hardware-optimize.service selfmicro-zram-config.service >/dev/null 2>&1
	exit 0
	EOF
	...
	for i in ${releases[@]}
	do
		echo "$(echo $i | sed 's/.*\///')=$(cat $i/support)" >> "${destination}"/etc/selfmicro-distribution-status
	done

	# armhwinfo, firstrun, selfmicromonitor, etc. config file
	cat <<-EOF > "${destination}"/etc/selfmicro-release
...
POST_FAMILY_TWEAKS_BSP
	...
	fingerprint_image "${destination}/etc/selfmicro.txt"
