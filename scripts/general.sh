	...
	case $1 in
		...
		if [[ -d "${DEB_STORAGE}" ]]; then
			...
			find "${DEB_STORAGE}" \( -name "${CHOSEN_KERNEL}_*.deb" -o \
				-name "selfmicro-*.deb" -o \
				-name "plymouth-theme-selfmicro_*.deb" -o \
			...
			[[ -n $RELEASE ]] && rm -f "${DEB_STORAGE}/${RELEASE}/selfmicro-desktop-${RELEASE}"_*.deb
		fi
		;;
...
exit_with_error()
{
	...
	exec {FD}>/var/lock/selfmicro-debootstrap-losetup
	flock -u "${FD}"
...
display_alert "Building kernel splash logo" "$RELEASE" "info"
	...
	"${SDCARD}"/lib/firmware/bootsplash.selfmicro >/dev/null 2>&1
	if [[ $BOOT_LOGO == yes || $BOOT_LOGO == desktop && $BUILD_DESKTOP == yes && $RELEASE != buster ]]; then
		[[ -f "${SDCARD}"/boot/selfmicroEnv.txt ]] &&	grep -q '^bootlogo' "${SDCARD}"/boot/selfmicroEnv.txt && \
		sed -i 's/^bootlogo.*/bootlogo=true/' "${SDCARD}"/boot/selfmicroEnv.txt || echo 'bootlogo=true' >> "${SDCARD}"/boot/selfmicroEnv.txt
...
# Installing debian packages in the selfmicro build system.
  ...
  if [[ $(dpkg --print-architecture) == amd64 ]]; then
	...
	*run after all host dependencies are installed*
	...
	if [[ ! -f "${USERPATCHES_PATH}"/README ]]; then
		...
		echo 'https:/www.selfmicro.org' >> "${USERPATCHES_PATH}"/README
...
#				sshpass -p ${PASS_ROOT} ssh root@${REMOTEIP} "dd bs=1k seek=8 if=/usr/lib/linux-u-boot-legacy-selfmicro400_2.1.0_arm64/boot0_sdcard.fex of=/dev/mmcblk0"
#				sshpass -p ${PASS_ROOT} ssh root@${REMOTEIP} "dd bs=1k seek=16400 if=/usr/lib/linux-u-boot-legacy-selfmicro400_2.1.0_arm64/boot_package.fex of=/dev/mmcblk0"
...
#			if [[ $BRANCH == current && $BOARD =~ selfmicrozero2|orangepi400 ]]; then
