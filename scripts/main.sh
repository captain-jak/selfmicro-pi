...
backtitle="Orange Pi building script, http://www.selfmicro.com	" 
titlestr="Choose an option"

# Warnings mitigation
[[ -z $LANGUAGE ]] && export LANGUAGE="fr_FR:fr"            # set to english if not set
...
if [[ $BUILD_OPT =~ rootfs|image && -z $BUILD_DESKTOP ]]; then
	...
	# read distribution support status which is written to the selfmicro-release file
...
#REPOSITORY_INSTALL="u-boot,kernel,bsp,selfmicro-zsh,selfmicro-config,selfmicro-firmware${BUILD_DESKTOP:+,selfmicro-desktop}"
...
BSP_CLI_PACKAGE_NAME="selfmicro-bsp-cli-${BOARD}"
BSP_CLI_PACKAGE_FULLNAME="${BSP_CLI_PACKAGE_NAME}_${REVISION}_${ARCH}"
BSP_DESKTOP_PACKAGE_NAME="selfmicro-bsp-desktop-${BOARD}"
...
CHOSEN_DESKTOP=selfmicro-${RELEASE}-desktop-${DESKTOP_ENVIRONMENT}
...
if [[ $BUILD_OPT == rootfs || $BUILD_OPT == image ]]; then
	...
	# Compile selfmicro-config if packed .deb does not exist or use the one from Orange Pi
	if [[ ! -f ${DEB_STORAGE}/selfmicro-config_${REVISION}_all.deb ]]; then
	
		[[ "${REPOSITORY_INSTALL}" != *selfmicro-config* ]] && compile_selfmicro-config
	fi 

	# Compile selfmicro-zsh if packed .deb does not exist or use the one from repository
	if [[ ! -f ${DEB_STORAGE}/selfmicro-zsh_${REVISION}_all.deb ]]; then

	        [[ "${REPOSITORY_INSTALL}" != *selfmicro-zsh* ]] && compile_selfmicro-zsh
	fi

	# Compile plymouth-theme-selfmicro if packed .deb does not exist or use the one from repository
	if [[ ! -f ${DEB_STORAGE}/plymouth-theme-selfmicro_${REVISION}_all.deb ]]; then

		[[ "${REPOSITORY_INSTALL}" != *plymouth-theme-selfmicro* ]] && compile_plymouth-theme-selfmicro
	fi

	# Compile selfmicro-firmware if packed .deb does not exist or use the one from repository
	if [[ "${REPOSITORY_INSTALL}" != *selfmicro-firmware* ]]; then

		if ! ls "${DEB_STORAGE}/selfmicro-firmware_${REVISION}_all.deb" 1> /dev/null 2>&1; then
		...
		#if ! ls "${DEB_STORAGE}/selfmicro-firmware-full_${REVISION}_all.deb" 1> /dev/null 2>&1; then
