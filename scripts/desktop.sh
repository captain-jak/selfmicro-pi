...
create_desktop_package ()
{
	...
	Section: xorg
	Priority: optional
	Recommends: ${DEBIAN_RECOMMENDS//[:space:]+/,}, selfmicro-bsp-desktop
	Provides: ${CHOSEN_DESKTOP}, selfmicro-${RELEASE}-desktop
	...
	# selfmicro create_desktop_package scripts

	unset aggregated_content

	mkdir -p "${destination}"/etc/selfmicro

	local aggregated_content=""
	aggregate_all_desktop "selfmicro/create_desktop_package.sh" $'\n'
...
create_bsp_desktop_package ()
{
	...
	# set up control file
	cat <<-EOF > "${destination}"/DEBIAN/control
	Package: selfmicro-bsp-desktop-${BOARD}
	Version: $REVISION
	Architecture: $ARCH
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Installed-Size: 1
	Section: xorg
	Priority: optional
	Provides: selfmicro-bsp-desktop, selfmicro-bsp-desktop-${BOARD}
	...
	local aggregated_content=""
	aggregate_all_desktop "debian/selfmicro-bsp-desktop/postinst" $'\n'
	...
	local aggregated_content=""
	aggregate_all_desktop "debian/selfmicro-bsp-desktop/prepare.sh" $'\n'
	eval "${aggregated_content}"
...
desktop_postinstall ()
{
	...
	if [[ $LINUXFAMILY == sun8i && $BRANCH == legacy ]]; then
		...
		echo "disp_mem_reserves=on" >> "${SDCARD}"/boot/selfmicroEnv.txt
		echo "extraargs=cma=96M" >> "${SDCARD}"/boot/selfmicroEnv.txt
	fi

	if [[ -d ${SDCARD}/etc/lightdm ]]; then
		mkdir -p ${SDCARD}/etc/lightdm/lightdm.conf.d
		cat <<-EOF > ${SDCARD}/etc/lightdm/lightdm.conf.d/22-selfmicro-autologin.conf
		[Seat:*]
