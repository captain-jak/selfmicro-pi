...
compile_uboot()
{
	...
	while read -r target; do
		...
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${toolchain2}:${PATH}" \
			...
			# selfmicro specifics u-boot settings
			[[ -f .config ]] && sed -i 's/CONFIG_LOCALVERSION=""/CONFIG_LOCALVERSION="-selfmicro"/g' .config
		...
		[[ -n $UBOOT_TOOLCHAIN2 ]] && cross_compile="SELFMICRO=foe"; # empty parameter is not allowed
	...
	Section: kernel
	Priority: optional
	Provides: selfmicro-u-boot
	Replaces: selfmicro-u-boot
	Conflicts: selfmicro-u-boot, u-boot-sunxi
...
compile_firmware()
{
	...
	plugin_dir="selfmicro-firmware${FULL}"
	mkdir -p "${firmwaretempdir}/${plugin_dir}/lib/firmware"

	[[ $IGNORE_UPDATES != yes ]] && fetch_from_repo "https://github.com/orangepi-xunlong/firmware" "${EXTER}/cache/sources/selfmicro-firmware-git" "branch:master"
	if [[ -n $FULL ]]; then
	...
	# cp : create hardlinks
	cp -af --reflink=auto "${EXTER}"/cache/sources/selfmicro-firmware-git/* "${firmwaretempdir}/${plugin_dir}/lib/firmware/"
	...
	Package: selfmicro-firmware${FULL}
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Installed-Size: 1
	Replaces: linux-firmware, firmware-brcm80211, firmware-ralink, firmware-samsung, firmware-realtek, selfmicro-firmware${REPLACE}
	Section: kernel
	Priority: optional
	...
	# pack
	mv "selfmicro-firmware${FULL}" "selfmicro-firmware${FULL}_${REVISION}_all"
	display_alert "Building firmware package" "selfmicro-firmware${FULL}_${REVISION}_all" "info"
	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "selfmicro-firmware${FULL}_${REVISION}_all" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
	mv "selfmicro-firmware${FULL}_${REVISION}_all" "selfmicro-firmware${FULL}"
	rsync -rq "selfmicro-firmware${FULL}_${REVISION}_all.deb" "${DEB_STORAGE}/"
...
compile_selfmicro-zsh()
{

	local tmp_dir selfmicro_zsh_dir
	tmp_dir=$(mktemp -d)
	chmod 700 ${tmp_dir}
	trap "rm -rf \"${tmp_dir}\" ; exit 0" 0 1 2 3 15
	selfmicro_zsh_dir=selfmicro-zsh_${REVISION}_all
	display_alert "Building deb" "selfmicro-zsh" "info"
	...
	mkdir -p "${tmp_dir}/${selfmicro_zsh_dir}"/{DEBIAN,etc/skel/,etc/oh-my-zsh/,/etc/skel/.oh-my-zsh/cache}

	# set up control file
	cat <<-END > "${tmp_dir}/${selfmicro_zsh_dir}"/DEBIAN/control
	Package: selfmicro-zsh
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Depends: zsh, tmux
	...
	cat <<-END > "${tmp_dir}/${selfmicro_zsh_dir}"/DEBIAN/postinst
	#!/bin/sh
	...
	cp -R "${EXTER}"/cache/sources/oh-my-zsh "${tmp_dir}/${selfmicro_zsh_dir}"/etc/
	cp -R "${EXTER}"/cache/sources/evalcache "${tmp_dir}/${selfmicro_zsh_dir}"/etc/oh-my-zsh/plugins
	cp "${tmp_dir}/${selfmicro_zsh_dir}"/etc/oh-my-zsh/templates/zshrc.zsh-template "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	chmod -R g-w,o-w "${tmp_dir}/${selfmicro_zsh_dir}"/etc/oh-my-zsh/

	# we have common settings
	sed -i "s/^export ZSH=.*/export ZSH=\/etc\/oh-my-zsh/" "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	# user cache
	sed -i "/^export ZSH=.*/a export ZSH_CACHE_DIR=~\/.oh-my-zsh\/cache" "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	# define theme
	sed -i 's/^ZSH_THEME=.*/ZSH_THEME="mrtazz"/' "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	# disable prompt while update
	sed -i 's/# DISABLE_UPDATE_PROMPT="true"/DISABLE_UPDATE_PROMPT="true"/g' "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	# disable auto update since we provide update via package
	sed -i 's/# DISABLE_AUTO_UPDATE="true"/DISABLE_AUTO_UPDATE="true"/g' "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	# define default plugins
	sed -i 's/^plugins=.*/plugins=(evalcache git git-extras debian tmux screen history extract colorize web-search docker)/' "${tmp_dir}/${selfmicro_zsh_dir}"/etc/skel/.zshrc

	chmod 755 "${tmp_dir}/${selfmicro_zsh_dir}"/DEBIAN/postinst

	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "${tmp_dir}/${selfmicro_zsh_dir}" >> "${DEST}"/${LOG_SUBPATH}/output.log 2>&1
	rsync --remove-source-files -rq "${tmp_dir}/${selfmicro_zsh_dir}.deb" "${DEB_STORAGE}/"
	rm -rf "${tmp_dir}"
...
compile_plymouth-theme-selfmicro()
{
	...
	plymouth_theme_selfmicro_dir=selfmicro-plymouth-theme_${REVISION}_all
	display_alert "Building deb" "selfmicro-plymouth-theme" "info"

	mkdir -p "${tmp_dir}/${plymouth_theme_selfmicro_dir}"/{DEBIAN,usr/share/plymouth/themes/selfmicro}

        # set up control file
	cat <<- END > "${tmp_dir}/${plymouth_theme_selfmicro_dir}"/DEBIAN/control
		Package: selfmicro-plymouth-theme
		Version: $REVISION
		Architecture: all
		Maintainer: $MAINTAINER <$MAINTAINERMAIL>
		...
		Priority: optional
		Description: boot animation, logger and I/O multiplexer - selfmicro theme
	END

	cp "${EXTER}"/packages/plymouth-theme-selfmicro/debian/{postinst,prerm,postrm} \
		"${tmp_dir}/${plymouth_theme_selfmicro_dir}"/DEBIAN/
	chmod 755 "${tmp_dir}/${plymouth_theme_selfmicro_dir}"/DEBIAN/{postinst,prerm,postrm}

	#convert -resize 256x256 \
	#	"${EXTER}"/packages/plymouth-theme-selfmicro/selfmicro-logo.png \
	#	"${tmp_dir}/${plymouth_theme_selfmicro_dir}"/usr/share/plymouth/themes/selfmicro/bgrt-fallback.png

	# convert -resize 52x52 \
	#       "${EXTER}"/packages/plymouth-theme-selfmicro/spinner.gif \
	#       "${tmp_dir}/${plymouth_theme_selfmicro_dir}"/usr/share/plymouth/themes/selfmicro/animation-%04d.png

	convert -resize 52x52 \
		"${EXTER}"/packages/plymouth-theme-selfmicro/spinner.gif \
		"${tmp_dir}/${plymouth_theme_selfmicro_dir}"/usr/share/plymouth/themes/selfmicro/throbber-%04d.png

	cp "${EXTER}"/packages/plymouth-theme-selfmicro/watermark.png \
		"${tmp_dir}/${plymouth_theme_selfmicro_dir}"/usr/share/plymouth/themes/selfmicro/

	cp "${EXTER}"/packages/plymouth-theme-selfmicro/{bullet,capslock,entry,keyboard,keymap-render,lock}.png \
		"${tmp_dir}/${plymouth_theme_selfmicro_dir}"/usr/share/plymouth/themes/selfmicro/

	cp "${EXTER}"/packages/plymouth-theme-selfmicro/selfmicro.plymouth \
		"${tmp_dir}/${plymouth_theme_selfmicro_dir}"/usr/share/plymouth/themes/selfmicro/

	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "${tmp_dir}/${plymouth_theme_selfmicro_dir}" > /dev/null
	rsync --remove-source-files -rq "${tmp_dir}/${plymouth_theme_selfmicro_dir}.deb" "${DEB_STORAGE}/"
	rm -rf "${tmp_dir}"
}

compile_selfmicro-config()
{
	local tmpdir=${SRC}/.tmp/selfmicro-config_${REVISION}_all

	display_alert "Building deb" "selfmicro-config" "info"


	mkdir -p "${tmpdir}"/{DEBIAN,usr/bin/,usr/sbin/,usr/lib/selfmicro-config/}

	# set up control file
	cat <<-END > "${tmpdir}"/DEBIAN/control
	Package: selfmicro-config
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Replaces: selfmicro-bsp
	...
	install -m 755 $EXTER/cache/sources/selfmicro-config/scripts/tv_grab_file $tmpdir/usr/bin/tv_grab_file
	install -m 755 $EXTER/cache/sources/selfmicro-config/debian-config $tmpdir/usr/sbin/selfmicro-config
	install -m 644 $EXTER/cache/sources/selfmicro-config/debian-config-jobs $tmpdir/usr/lib/selfmicro-config/jobs.sh
	install -m 644 $EXTER/cache/sources/selfmicro-config/debian-config-submenu $tmpdir/usr/lib/selfmicro-config/submenu.sh
	install -m 644 $EXTER/cache/sources/selfmicro-config/debian-config-functions $tmpdir/usr/lib/selfmicro-config/functions.sh
	install -m 644 $EXTER/cache/sources/selfmicro-config/debian-config-functions-network $tmpdir/usr/lib/selfmicro-config/functions-network.sh
	install -m 755 $EXTER/cache/sources/selfmicro-config/softy $tmpdir/usr/sbin/softy
	# fallback to replace selfmicro-config in BSP
	ln -sf /usr/sbin/selfmicro-config $tmpdir/usr/bin/selfmicro-config
...
# <board>: selfmicropcplus, selfmicrozero ...
...
userpatch_create()
{
	...
	if ! git diff-index --quiet --cached HEAD; then
		...
		if [[ -n $(git config user.email) ]]; then
			...
			git format-patch -1 HEAD --stdout --signature="Created with selfmicro build tools https://github.com/orangepi-xunlong/build" > "${patch}"
...
overlayfs_wrapper()
{
	...
	if [[ $operation == wrap ]]; then
		local srcdir="$2"
		local description="$3"
		mkdir -p /tmp/overlay_components/ /tmp/selfmicro_build/
		...
		mergeddir=$(mktemp -d --suffix="_$description" --tmpdir="/tmp/selfmicro_build/")
