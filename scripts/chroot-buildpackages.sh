...
create_build_script ()
{
	...
	# set local version
	# debchange -l~selfmicro${REVISION}-${builddate}+ "Custom $VENDOR release"
	debchange -l~selfmicro"${REVISION}"+ "Custom $VENDOR release"
...
chroot_installpackages_local()
{
	...
	if [[ $EXTERNAL_NEW == prebuilt ]]; then
		aptly -config="${conf}" repo add temp "${DEB_SELFMICRO}/extra/${RELEASE}-desktop/" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
		aptly -config="${conf}" repo add temp "${DEB_SELFMICRO}/extra/${RELEASE}-utils/" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
	else
	...
	cat <<-'EOF' > "${SDCARD}"/etc/apt/preferences.d/90-selfmicro-temp.pref
	Package: *
	Pin: origin "localhost"
	Pin-Priority: 550
	EOF
	cat <<-EOF > "${SDCARD}"/etc/apt/sources.list.d/selfmicro-temp.list
...
chroot_installpackages()
{
	...
	rm /etc/apt/sources.list.d/selfmicro-temp.list 2>/dev/null
	rm /etc/apt/preferences.d/90-selfmicro-temp.pref 2>/dev/null
	...
	[[ -f ${SDCARD}/etc/hostapd.conf ]] && sed -i "s/^ssid=.*/ssid=selfmicro/" ${SDCARD}/etc/hostapd.conf
} #############################################################################
